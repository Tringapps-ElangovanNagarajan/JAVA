...
public class PlayerTwo implements PlayerStrategy{
	...
	public void receiveCard(Card drawnCard) {
		System.out.println(" Second Player recieved :"+drawnCard.getRank()+" "+drawnCard.getSuit());
	...
	public Card playCard() {
		...
		if(changedSuit==null) {
			for(int i=0; i<myCards.size() ;i++) {
				if(myCards.get(i).getSuit().equals(topPileCard.getSuit())||myCards.get(i).getRank().equals(topPileCard.getRank())) {
					System.out.println("Second Player played: "+myCards.get(i).getRank()+" "+myCards.get(i).getSuit());
	...
	public Card.Suit declareSuit(){
		...
		for(int i=0; i<myCards.size() ;i++) {
			...
			if(count < max) {
				max=count;
				Decksiut=myCards.get(i);
			}
		}
		System.out.println("Delcare suit: "+Decksiut.getSuit());
		return Decksiut.getSuit();

...
public class PlayerOne implements PlayerStrategy{
	...
	public void receiveCard(Card drawnCard) {
		System.out.println("First Player recieved :"+drawnCard.getRank()+" "+drawnCard.getSuit());
	...
	public Card playCard() {
		...
		if(changedSuit==null) {
			for(int i=0;i<myCards.size();i++) {
				if(myCards.get(i).getSuit().equals(topPileCard.getSuit())||myCards.get(i).getRank().equals(topPileCard.getRank())) {
					System.out.println("First Player played: "+myCards.get(i).getRank()+" "+myCards.get(i).getSuit());
